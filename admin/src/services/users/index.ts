...
export const signIn = async ({
  email,
  password,
}: ISignInParams): Promise<ISignInReponse> => {
...
export const updateUser = async (
  user_id: string,
  data: Omit<IUserFormData, "password">
) => {
  const token = getAccessTokenFromLocalStorage();
  ...
  if (response.data.status === "success") {
    if (avatarFile instanceof File || avatarFile instanceof Blob) {
      ...
      const oldAvatarName: string =
        response.data.data?.addition?.old_avatar || "";
...
export const deleteUser = async (userId: string) => {
  ...
  const response = await api.post("/users/delete/" + userId, null, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
