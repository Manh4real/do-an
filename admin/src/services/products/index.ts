...
import { deleteProductImage, uploadProductImage as uploadProductImage_firebase } from '../../features/firebase';
        ...
        if (newProduct) {
            ...
            Object.entries(imageFiles).forEach(([colorId, images]) => {
                for (let i = 0; i < images.length; i++) {
                    ...
                    const b = {
                        url: imageName, product_id: newProduct.product_id, product_color_id: colorId
                    };
...
export const updateProduct = async (product_id: string, data: IProductEditFormData) => {
    const token = getAccessTokenFromLocalStorage();
    ...
    if (removedImages) {
        ...
        const imageIds = Object.entries(removedImages).reduce((acc, [_, images]) => {
...
export const deleteProduct = async (productId: string) => {
    ...
    const response = await api.post("/delete/products/" + productId, null, {
        headers: {
            Authorization: `Bearer ${token}`
        }
    })
