...
export const getManufacturers = async (page = 1, options: AxiosRequestConfig<{ signal: GenericAbortSignal }> = {}) => {
    try {
...
export const deleteManufacturer = async (manufacturerId: string, options: AxiosRequestConfig<{ signal: GenericAbortSignal }> = {}) => {
    try {
        const token = getAccessTokenFromLocalStorage();
        const response = await api.post("/delete/manufacturers/" + manufacturerId, null, {
            headers: {
...
export const updateManufacturer = async (manufacturerId: string, data: IManufacturerFormData) => {
    try {
        const token = getAccessTokenFromLocalStorage();
